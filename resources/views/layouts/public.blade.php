...
<head>
    ...
    <title>{{ $title ?? (config('app.name', 'கதைங்கோ') . ' - Inspiring Stories & Ideas') }}</title>
    ...
    <link href="https://fonts.googleapis.com/css2?family=DM+Sans:ital,opsz,wght@0,9..40,100..1000;1,9..40,100..1000&family=Marcellus&family=Inter:wght@300;400;500;600;700;800;900&display=swap" rel="stylesheet">
...
<body class="antialiased bg-slate-gray text-white flex flex-col min-h-screen">
    ...
    <!-- Global Footer -->
    <footer class="bg-gray-900 border-t border-gray-800 py-16 px-6 lg:px-8 mt-auto">
        <div class="max-w-7xl mx-auto">
            <div class="grid md:grid-cols-2 lg:grid-cols-5 gap-12 mb-12">
                <div class="lg:col-span-2 text-left">
                    <div class="mb-6">
                        <img src="{{ asset('images/logo/logo-footer.png') }}" alt="கதைங்கோ" class="h-16 w-auto">
                    </div>
                        ...
                        <div class="text-left">
                            ...
                            <ul class="space-y-2 p-0 list-none">
                                @foreach($category->subcategories->take(4) as $subcategory)
                                    <li>
                                        @php
                                            $subLink = $category->slug === 'pathivargal'
                                                ? route('authors.show', $subcategory->slug)
                                                : url('/stories?subcategory=' . $subcategory->slug);
                                        @endphp
                                        <a href="{{ $subLink }}" class="text-gray-400 hover:text-burnt-orange text-sm transition">
                                            {{ $subcategory->name }}
                                        </a>
                                    </li>
                                @endforeach
