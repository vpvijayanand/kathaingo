        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-gray-900 border border-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-white">
                    <form method="POST" action="{{ route('posts.update', $post->id) }}" enctype="multipart/form-data" x-data="postTaxonomy()" id="post-form" @submit.prevent="handleFormSubmit($event)">
                        @csrf
                        @method('PUT')
                        ...
                        <div x-show="hasAutosave" x-transition class="mb-6 bg-orange-950/30 border border-burnt-orange/40 text-orange-400 p-4 rounded-xl flex items-center justify-between shadow-lg" x-cloak>
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm">நாங்கள் இந்த மாற்றத்தின் சேமிக்கப்படாத நகலைக் கண்டறிந்துள்ளோம் (<span x-text="autosaveTime"></span>). We found unsaved local changes.</span>
                            </div>
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="title" class="block mt-1 w-full" type="text" name="title"
                                :value="old('title', $post->title)" required autofocus data-kathaingo-transliterate="true" />
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="excerpt" name="excerpt"
                                class="block mt-1 w-full bg-gray-950 text-white border-gray-800 focus:border-burnt-orange focus:ring-burnt-orange rounded-md shadow-sm"
                                rows="3" data-kathaingo-transliterate="true">{{ old('excerpt', $post->excerpt) }}</textarea>
                        ...
                        @if($post->featured_image)
                            <div class="mb-4">
                                <x-input-label :value="__('Current Featured Image')" />
                                <img src="{{ asset('storage/' . $post->featured_image) }}" alt="Current featured image"
                                    class="mt-2 max-w-xs rounded-md shadow-sm bg-gray-950 border border-gray-850">
                            </div>
                        @endif

                        <!-- Featured Image Upload -->
                        <div class="mb-4">
                            <x-input-label for="featured_image" :value="__('Featured Image ' . ($post->featured_image ? '(Replace)' : ''))" />
                            ...
                            <p class="mt-1 text-sm text-gray-400">Upload a new featured image (JPG, PNG, GIF - Max 2MB)
                            </p>
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="image" class="block mt-1 w-full" type="url" name="image"
                                :value="old('image', $post->image)" placeholder="https://example.com/image.jpg" />
                            ...
                            <div class="mb-4">
                                ...
                                <select id="author_subcategory_id" name="author_subcategory_id" required
                                    ...
                                    @foreach($authors as $author)
                                        <option value="{{ $author->id }}" {{ old('author_subcategory_id', $post->author_subcategory_id) == $author->id ? 'selected' : '' }}>
                                            {{ $author->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div>
                            ...
                            <div x-data="{ open: {{ ($errors->has('category_id') || $errors->has('metadata_value_ids') || $errors->has('metadata_value_ids.*') || old('category_id') || old('metadata_value_ids')) ? 'true' : 'false' }} }" class="mb-6 border border-gray-800 rounded-xl bg-gray-950/20 overflow-hidden">
                                ...
                                <div x-show="open" 
                                     ...
                                     class="p-4 border-t border-gray-800 bg-gray-955/40"
                                     x-cloak>
                             ...
                             <div class="p-4 bg-gray-950 border border-gray-800 rounded-xl mb-4" x-data="{
                                 inSeries: {{ old('series_id', $post->series_id) ? 'true' : 'false' }},
                                 ...
                                 <div x-show="inSeries" class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
                                     ...
                                     <div>
                                         ...
                                         <x-text-input id="volume" name="volume" class="block mt-1 w-full" type="text" :value="old('volume', $post->volume)" placeholder="e.g. Volume 1, Part 2" />
                                     </div>
                                     <div>
                                         <x-input-label for="chapter_number" :value="__('அத்தியாய எண் / Chapter Number (Optional)')" />
                                         <x-text-input id="chapter_number" name="chapter_number" class="block mt-1 w-full" type="number" :value="old('chapter_number', $post->chapter_number)" placeholder="e.g. 1, 2" />
                                     </div>
                                     ...
                                     <div x-show="showNewSeriesForm" x-transition class="col-span-1 md:col-span-3 mt-2 p-4 bg-gray-900 border border-gray-850 rounded-xl">
                                         ...
                                         <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                             ...
                                             <div>
                                                 ...
                                                 <input type="text" x-model="newSeriesDesc" class="block w-full bg-gray-950 text-white border-gray-800 focus:border-burnt-orange focus:ring-burnt-orange rounded-md shadow-sm text-sm" placeholder="e.g. ஒரு சுருக்கமான விளக்கம்">
                                             </div>
                                         </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select id="country_code" name="country_code"
                                ...
                                @foreach(\App\Helpers\CountryHelper::getCountries() as $code => $country)
                                    <option value="{{ $code }}" {{ old('country_code', $post->country_code) == $code ? 'selected' : '' }}>
                        ...
                        <script>
                            function postTaxonomy() {
                                return {
                                    ...
                                    selectedMetadataValues: @json(array_map('intval', old('metadata_value_ids', $post->metadataValues->pluck('id')->toArray()))),
                                    ...
                                    getAutoSaveKey() {
                                        return 'kathaingo_draft_autosave_edit_{{ $post->id }}';
                                    },
                                    ...
                                    runPreview() {
                                        ...
                                        if (fileEl && fileEl.files && fileEl.files[0]) {
                                            ...
                                            // Pre-existing post image if editing
                                            this.previewData.imageSrc = '{{ $post->image ? $post->image : ($post->featured_image ? asset("storage/" . $post->featured_image) : "") }}';
                                        }
                                    ...
                                    runReviewReport() {
                                        ...
                                        const hasImage = (fileEl && fileEl.files && fileEl.files.length > 0) || urlVal !== '' || '{{ $post->image || $post->featured_image ? "1" : "" }}' !== '';
                        ...
                        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mt-8 border-t border-gray-800 pt-6">
                            ...
                            <div class="w-full md:w-64">
                                ...
                                <select id="status" name="status" x-model="selectedStatus"
                                    ...
                                    @endphp
                                    @if($isAdminOrEditor)
                                        <option value="draft" {{ old('status', $post->status) == 'draft' ? 'selected' : '' }}>Draft (வரைவு)</option>
                                        <option value="submitted" {{ old('status', $post->status) == 'submitted' ? 'selected' : '' }}>Submit for Review (மதிப்பாய்வுக்கு அனுப்பு)</option>
                                        <option value="under_review" {{ old('status', $post->status) == 'under_review' ? 'selected' : '' }}>Under Review (மதிப்பாய்வில் உள்ளது)</option>
                                        <option value="approved" {{ old('status', $post->status) == 'approved' ? 'selected' : '' }}>Approved (அங்கீகரிக்கப்பட்டது)</option>
                                        <option value="published" {{ old('status', $post->status) == 'published' ? 'selected' : '' }}>Published (வெளியிடப்பட்டது)</option>
                                        <option value="rejected" {{ old('status', $post->status) == 'rejected' ? 'selected' : '' }}>Rejected (நிராகரிக்கப்பட்டது)</option>
                                    @elseif($trustLevel >= 3)
                                        <option value="draft" {{ old('status', $post->status) == 'draft' ? 'selected' : '' }}>Draft (வரைவு)</option>
                                        <option value="published" {{ old('status', $post->status) == 'published' ? 'selected' : '' }}>Publish (வெளியிடு)</option>
                                    @elseif($trustLevel == 2)
                                        <option value="draft" {{ old('status', $post->status) == 'draft' ? 'selected' : '' }}>Draft (வரைவு)</option>
                                        <option value="submitted" {{ old('status', $post->status) == 'submitted' ? 'selected' : '' }}>Submit for Review (மதிப்பாய்வுக்கு அனுப்பு)</option>
                                        <option value="published" {{ old('status', $post->status) == 'published' ? 'selected' : '' }}>Publish (வெளியிடு)</option>
                            ...
                            <div class="flex flex-col items-stretch sm:items-end gap-2 w-full md:w-auto">
                                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                                    ...
                                    <button type="submit"
                                        ...
                                        x-text="selectedStatus === 'published' ? '{{ __('Publish') }}' : (selectedStatus === 'submitted' ? '{{ __('Submit for Review') }}' : (selectedStatus === 'draft' ? '{{ __('Save as Draft') }}' : '{{ __('Update Post') }}'))">
                                        {{ __('Update Post') }}
                                    </button>
                                </div>
